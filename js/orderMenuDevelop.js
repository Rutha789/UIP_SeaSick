...
function initialOrderMenu() {
  ...
  orderBarPromise.then(function() {
    undoManager.registerCallback(() =>
      clickIf($("#confirmButton"), orderList.length() > 0, renderPaymentScreen)
    );
  ...
  $("#toggle-filter-btn").click(function() {
    $("#filter-menu").css("display", (i, display) =>
...
function nextPage() {
  ...
  let filteredMenu = drinkMenuModel
    .getMenu()
...
function setMainCategory(category) {
  if (
    typeof drinkMenuModel === "undefined" ||
...
function submitFiltering() {
  let action = drinkMenuModel.modifyFilterCommand(getFilterForm).augment(
    new Command(
      function() {
        pageIx = 0;
      },
